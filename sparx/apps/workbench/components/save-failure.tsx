...
//
// A hundred-odd surfaces in each console had hand-rolled the same six lines — a
// conditional `<Alert color="error">` around a title and a message — and every
// one of them rendered at the TOP of a scrolling pane while the button that
// triggered it sits pinned in the toolbar. On a form taller than the pane,
// pressing Save produced a refusal 700px above the fold and no other change on
// screen. From the seat, that reads as a dead button.
//
// Found in the other console by a persona and repaired there first. This is its
// OWN copy, not an import: neither brand tree may depend on the other, and
// check:console-parity holds the two level rather than joining them.
...
export function SaveFailure({
  ...
  // very field it named — the same "message outliving its cause" the failed-write
  // reporter already guards against, moved from the toast into the pane.
