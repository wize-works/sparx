...
- [ ] **3.3c — cleanup** (next): remove the `page_key` alias from routes/service/input schemas/`SectionView`; cut MCP tools to `templateId`/`scope+key`; relocate the legacy-pageKey mapping to the storefront snapshot read path only.
