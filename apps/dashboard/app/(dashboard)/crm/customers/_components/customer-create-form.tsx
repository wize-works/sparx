...
// New-customer form, surface-aware (§13.1). The SAME component renders:
//   - `surface="page"`   inside the /new route's Container + PageHeader
//   - `surface="overlay"` inside the `@detail` drawer/modal chrome
//
// The action consumes the structured object directly (not FormData) because
// @sparx/crm-schemas owns the validation contract — we let Zod do its work
// server-side. On success the overlay swaps to the new record's detail view
// (create flows into view); the page pushes to the record.

interface CustomerCreateFormProps {
  surface: 'page' | 'overlay';
}

export function CustomerCreateForm({ surface }: CustomerCreateFormProps) {
          ...
          <CardContent>
            <Stack gap={4}>
              ...
              <Stack direction="row" align="center" gap={2}>
                <input type="checkbox" id="doNotContact" name="doNotContact" className="h-4 w-4" />
