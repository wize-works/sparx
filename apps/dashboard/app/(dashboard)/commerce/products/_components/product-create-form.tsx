...
// New-product form, surface-aware (§13.1). The SAME component renders:
//   - `surface="page"`   inside the /new route's Container + PageHeader
//   - `surface="overlay"` inside the `@detail` drawer/modal chrome
//
// Phase 1.1 captures only the catalog basics — variants, options, fitment,
// media, pricing each land in their own tabs on the product detail page once
// the row exists. Only the framing (internal heading, Cancel target) and the
// post-create transition differ by surface; the fields are identical. On
// success the overlay swaps to the new record's detail view (create flows into
...
export function ProductCreateForm({ surface }: ProductCreateFormProps) {
              ...
              <Stack gap={4}>
                ...
                <Stack gap={2}>
                  ...
                  <Text size="xs" variant="muted">
                    Storefront path: <code>/products/&lt;handle&gt;</code>. Lowercase, dashes only.
                  </Text>
