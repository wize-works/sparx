...
import type { ConsumerContext } from './registry.js';
import { gateHandler } from './registry.js';
...
export function registerOrderEventConsumers(ctx: ConsumerContext): Array<() => void> {
  return [
    ctx.bus.subscribe(
      'order.created',
  ...
  await withTenant({ tenantId }, async (tx) => {
    await tx.crmActivity.create({
      data: {
        ...
        linkedEntityType: 'Order',
        linkedEntityId: payload.orderId,
        metadata: { orderId: payload.orderId } as Prisma.InputJsonValue,
      },
    });
  });
}
